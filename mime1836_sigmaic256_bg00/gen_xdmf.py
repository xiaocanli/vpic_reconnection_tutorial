...
delta_fields = False  # whether analyzing fluctuation of fields
...
def scalar_field(field, species, dims, tindex, grid2):
    ...
    if data_format == "Binary":
        if smoothed_data:
            dataitem.text = dir_smooth_data + '/' + var_component + "_" + str(tindex) + ".gda"
        else:
            dataitem.text = "data/" + var_component + "_" + str(tindex) + ".gda"
    else:
        if delta_fields:
            if var_component in ["bx", "by", "bz"]:
                fname = "dB_" + str(tindex) + ".h5"
            elif var_component in ["vx", "vy", "vz"]:
                fname = "dV_" + str(tindex) + ".h5"
        else:
        ...
        if smoothed_data:
            dataitem.text = (dir_smooth_data + "/" + fname + ":/Timestep_" +
                             str(tindex) + "/" + var_component)
        else:
            if var_component in ["cbx", "cby", "cbz", "ex", "ey", "ez"]:
                fdir = "field_hdf5/"
            else:
...
def fields_list_binary():
    ...
    if compound_data:
        ...
        #                {"name": "ne", "vars": ["ne"]},
        #                {"name": "ni", "vars": ["ni"]}]
        fields_list = [{"name": "V", "vars": ["Vx", "Vy", "Vz"]},
                       {"name": "B", "vars": ["Bx", "By", "Bz"]}]
    else:
...
def main():
    ...
    sfactor = smooth_factor if smoothed_data else 1
    if data_format == "Binary":
        dims = (str(int(vpic_info["nz"]/sfactor)) + " " +
                str(int(vpic_info["ny"]/sfactor)) + " " +
                str(int(vpic_info["nx"]/sfactor)))
        origin = (str(-0.5*vpic_info["Lz/de"]) + " " +
                  str(-0.5*vpic_info["Ly/de"]) + " 0")
        dxdydz = (str(vpic_info["dz/de"]*sfactor) + " " +
                  str(vpic_info["dy/de"]*sfactor) + " " +
                  str(vpic_info["dx/de"]*sfactor))
    else:
        dims = (str(int(vpic_info["nx"]/sfactor)) + " " +
                str(int(vpic_info["ny"]/sfactor)) + " " +
                str(int(vpic_info["nz"]/sfactor)))
        origin = ("0 " +
                  str(-0.5*vpic_info["Ly/de"]) + " " +
                  str(-0.5*vpic_info["Lz/de"]))
        dxdydz = (str(vpic_info["dx/de"]*sfactor) + " " +
                  str(vpic_info["dy/de"]*sfactor) + " " +
                  str(vpic_info["dz/de"]*sfactor))
    ...
    if delta_fields:
        with open('./delta_vb.xdmf', 'wb') as f:
