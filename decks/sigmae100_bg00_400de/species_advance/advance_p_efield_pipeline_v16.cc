...
// advance particles with part of electric field
void
advance_p_efield_pipeline_v16( advance_p_efield_pipeline_args_t * args,
                               int pipeline_rank,
                               int n_pipeline )
{
  ...
  const v16float gamma_sironi(args->gamma_sironi);
  const v16float four(4.0);
  const v16float one(1.0);
  const v16float one_half(0.5);
  const v16float one_third(1.0/3.0);
  const v16float one_fourth(0.25);
  const v16float two_fifteenths(2.0/15.0);
  const v16float neg_one(-1.0);
  const v16float zeros(0.0);
  const v16float ten(10.0);
  const v16float one_hundred(100.0);
  ...
  v16float ex, ey, ez;
  v16float eparax, eparay, eparaz, ie2b2, w2, wtmp;
  v16float edotb, etot2, btot2, ib2;
  v16float gamma_scale;
  v16int   ii, outbnd, valid_number;
  ...
  for( ; nq; nq--, p+=16 )
  {
    ...
    ex = fma( fma( v02, dy, v01 ), dz, fma( v00, dy, hax ) );

    ey = fma( fma( v05, dz, v04 ), dx, fma( v03, dz, hay ) );

    ez = fma( fma( v08, dx, v07 ), dy, fma( v06, dx, haz ) );
    ...
    //--------------------------------------------------------------------------
    // Get part of the electric field
    //--------------------------------------------------------------------------
    btot2 = fma(cbx, cbx, fma(cby, cby, cbz*cbz));
    ib2 = rcp(btot2);
    edotb = fma(ex, cbx, fma(ey, cby, ez*cbz));
    ...
    if (etype == 0) { // remove all parallel electric field
      ...
      etot2 = fma(ex, ex, fma(ey, ey, ez*ez));
      outbnd = etot2 > btot2;
      eparax  = qdt_2mc * ex; // reuse eparax here for updated hax
      eparay  = qdt_2mc * ey; // reuse eparay here for updated hay
      eparaz  = qdt_2mc * ez; // reuse eparaz here for updated haz
      hax = merge( outbnd, zeros, eparax ); // zeros for outbnd particles
      hay = merge( outbnd, zeros, eparay );
      haz = merge( outbnd, zeros, eparaz );
      if (etype == 3 || etype == 4) { // set gamma to a few (Sironi's method)
        v00 = rsqrt( one + fma( ux, ux, fma( uy, uy, uz*uz ) ) );
        gamma_scale = gamma_sironi * v00 / msp;
        if (etype == 4) {
          outbnd = outbnd && (q < ten);
        }
        ux = merge( outbnd, ux*gamma_scale, ux );
        uy = merge( outbnd, uy*gamma_scale, uy );
        uz = merge( outbnd, uz*gamma_scale, uz );
      }
      q = merge( outbnd, one_hundred, q );
    ...
    if (etype == 5) { // remove perpendicular electric field but keep ExB drift
      /* eparax = fms(ey, cbz, ez * cby) * ib2; // reuse epara as ExB drift */
      /* eparay = fms(ez, cbx, ex * cbz) * ib2; */
      /* eparaz = fms(ex, cby, ey * cbx) * ib2; */
      // https://aip.scitation.org/doi/10.1063/1.873736 for calculating ExB drift
      w2 = fma(ex, ex, fma(ey, ey, ez*ez)) + fma(cbx, cbx, fma(cby, cby, cbz*cbz));
      valid_number = (w2 != zeros);
      ie2b2 = merge(valid_number, rcp(w2), zeros);
      eparax = fms(ey, cbz, ez * cby) * ie2b2; // reuse epara as ExB/(E^2+B^2)
      eparay = fms(ez, cbx, ex * cbz) * ie2b2;
      eparaz = fms(ex, cby, ey * cbx) * ie2b2;
      w2 = fma(eparax, eparax, fma(eparay, eparay, eparaz*eparaz));
      valid_number = (w2 <= one_fourth);
      wtmp = merge(valid_number, sqrt(one-four*w2), zeros);
      valid_number = (w2 != zeros);
      w2 = merge(valid_number, one_half * (one-wtmp) * rcp(w2), zeros); // reuse w2
                               ...
                               int n_pipeline )
{
  ...
  ERROR( ( "No advance_p_efield_pipeline_v16 implementation." ) );
}
