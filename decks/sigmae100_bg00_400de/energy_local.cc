  ...
  if (should_dump(spectrum)) {
    ...
    for (int isp = 0; isp < nsp; isp++) {   // loop over species
...
#endif
      for (int zone_z = 0; zone_z < nzones_z; zone_z++) {
        for (int zone_y = 0; zone_y < nzones_y; zone_y++) {
          for (int zone_x = 0; zone_x < nzones_x; zone_x++) {
            int zone = zone_x + nzones_x * (zone_y + zone_z * nzones_y);
            local_spectrum[zone*(nbins+3)  ] = bx_avg[zone];
            local_spectrum[zone*(nbins+3)+1] = by_avg[zone];
            local_spectrum[zone*(nbins+3)+2] = bz_avg[zone];
#ifdef CHECK_PARTICLE_CROSSING_EGTB
            local_spectrum_egtb[zone*(nbins+3)  ] = bx_avg[zone];
            local_spectrum_egtb[zone*(nbins+3)+1] = by_avg[zone];
            local_spectrum_egtb[zone*(nbins+3)+2] = bz_avg[zone];
#endif
          }
        }
      }
      ...
      for (std::size_t i = 0; i < sp->np; ++i ) { // loop over particles
        ...
        int zone_z = (p->i / (nx2 * ny2) - 1) / global->nz_zone;
        int zone_y = ((p->i % (nx2 * ny2)) / nx2 - 1) / global->ny_zone;
        int zone_x = (p->i % nx2 - 1) / global->nx_zone;
        int zone = zone_x + nzones_x * (zone_y + zone_z * nzones_y);
