                              ...
                              int n_pipeline )
{
  ...
  const v4float gamma_sironi(args->gamma_sironi);
  const v4float four(4.0);
  const v4float one(1.0);
  const v4float one_half(0.5);
  const v4float one_third(1.0/3.0);
  const v4float one_fourth(0.25);
  const v4float two_fifteenths(2.0/15.0);
  const v4float neg_one(-1.0);
  const v4float zeros(0.0);
  const v4float ten(10.0);
  const v4float one_hundred(100.0);
  ...
  v4float ex, ey, ez;
  v4float eparax, eparay, eparaz, ie2b2, w2, wtmp;
  v4float edotb, etot2, btot2, ib2;
  v4float gamma_scale;
  v4int   ii, outbnd, valid_number;
  ...
  for( ; nq; nq--, p+=4 )
  {
    ...
    ez = fma( fma( v02, dx, v01 ), dy, fma( v00, dx, haz ) );
                              ...
                              int n_pipeline )
{
  ...
  ERROR( ( "No advance_p_efield_pipeline_v4 implementation." ) );
}
