...
#include "test_particle/tracer.hh"
...
begin_initialization {
  ...
  int particle_select = 30000; // track one every particle_select particles
  ...
  double sigmai = 102.4; // Cold ion magnetization
  double sigma = mi_me * sigmai; // Magnetization factor (2*magnetic energy)/(electron rest energy)
...
#ifdef RELATIVISTIC_PLASMA
  double Te      = 10*sigmai;   // Electron temperature
#else
  ...
  double taui    = 300.0; // simulation wci's to run
  ...
  int nx_zone = 384;             // # of cells / zone along x for energy spectrum diagnostics
  ...
  int nz_zone = 160;           // # of cells / zone along z for energy spectrum diagnostics
                                // MAKE SURE that (nz / topology_z) is divisible by nz_zone

  // particle dump
  int stride_particle_dump = 1; // stride for particle dump
...
begin_diagnostics {
  ...
  // move tracer particles to device and uncenter them
  if (step() == 0) {
    species_t * sp;
    if (global->particle_tracing == 1) {
      ...
      if( global->tracers_list ) {
        if( rank()==0 ) MESSAGE(( "Uncentering tracer particles" ));
...
#define TRACER_USER_DEFINED_DATA                                \
  ...
  if ( global->particle_tracing==1 ) {
    if ( should_dump(tracer) || step() == num_step) {
#include "test_particle/dumptracer_hdf5_single.cc"
    } // if
  }
...
#include "energy_local_kokkos.cc"   // Subroutine to compute energy spectrum diagnostic
...
#include "test_particle/dumptracer_hdf5_single.cc"
  ...
  if ( step() == num_step ) {
    ...
    if ( global->particle_tracing > 0) {
      ...
      delete_species_list( global->tracers_list );
    }
  }
