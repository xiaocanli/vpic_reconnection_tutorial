...
#define advance_tracers(RESTART_TAG) BEGIN_PRIMITIVE{                    \
    ...
    LIST_FOR_EACH( sp, global->tracers_list )                            \
    {                                                                    \
    ...
    LIST_FOR_EACH( sp, global->tracers_list )                            \
    {                                                                    \
      advance_p_test_particle(sp, interpolator_array, field_array);      \
    }                                                                    \
    LIST_FOR_EACH( sp, global->tracers_list )                            \
    {                                                                    \
    ...
    {                                                                    \
      boundary_p_test_particle_kokkos( particle_bc_list,                 \
          global->tracers_list, field_array );                           \
    }                                                                    \
    LIST_FOR_EACH( sp, global->tracers_list )                            \
    {                                                                    \
...
#define f_cbx interpolators_k(ii, interpolator_var::cbx)
#define f_cby interpolators_k(ii, interpolator_var::cby)
#define f_cbz interpolators_k(ii, interpolator_var::cbz)
#define f_ex  interpolators_k(ii, interpolator_var::ex)
#define f_ey  interpolators_k(ii, interpolator_var::ey)
#define f_ez  interpolators_k(ii, interpolator_var::ez)

#define f_dexdy    interpolators_k(ii, interpolator_var::dexdy)
#define f_dexdz    interpolators_k(ii, interpolator_var::dexdz)

#define f_d2exdydz interpolators_k(ii, interpolator_var::d2exdydz)
#define f_deydx    interpolators_k(ii, interpolator_var::deydx)
#define f_deydz    interpolators_k(ii, interpolator_var::deydz)

#define f_d2eydzdx interpolators_k(ii, interpolator_var::d2eydzdx)
#define f_dezdx    interpolators_k(ii, interpolator_var::dezdx)
#define f_dezdy    interpolators_k(ii, interpolator_var::dezdy)

#define f_d2ezdxdy interpolators_k(ii, interpolator_var::d2ezdxdy)
#define f_dcbxdx   interpolators_k(ii, interpolator_var::dcbxdx)
#define f_dcbydy   interpolators_k(ii, interpolator_var::dcbydy)
#define f_dcbzdz   interpolators_k(ii, interpolator_var::dcbzdz)

//--------------------------------------------------------------
// Electric and magnetic fields at tracer positions
#ifndef CALC_EMFS_AT_TRACER
#define CALC_EMFS_AT_TRACER                                                 \
    ex  = (f_ex + dy0*f_dexdy + dz0*(f_dexdz+dy0*f_d2exdydz));              \
    ey  = (f_ey + dz0*f_deydz + dx0*(f_deydx+dz0*f_d2eydzdx));              \
    ez  = (f_ez + dx0*f_dezdx + dy0*(f_dezdy+dx0*f_d2ezdxdy));              \
    bx  = (f_cbx + dx0*f_dcbxdx);                                           \
    by  = (f_cby + dy0*f_dcbydy);                                           \
    bz  = (f_cbz + dz0*f_dcbzdz)
#endif
