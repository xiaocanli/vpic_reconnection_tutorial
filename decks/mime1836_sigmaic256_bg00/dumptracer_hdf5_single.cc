{
  ...
  static double time_getting_tracer = 0.0;
  double t1 = uptime();
  while (sp) {
  ...
  if (step() == num_step || to_dump_tracer || reach_quota_time) {
    ...
    sim_log("Time in getting tracer information: "<< time_getting_tracer << " s");
    time_getting_tracer = 0.0; // reset time for get tracer information

    sim_log("Time in accumulating hydro for tracer: "<< time_accumulating_hydro << " s");
    time_accumulating_hydro = 0.0; // reset time for accumulating hydro for tracer
